...
int8_t pulseOximeter_readRegister(uint8_t reg, uint8_t* value)
{
	...
	retStatus = HAL_I2C_Master_Transmit(&hi2c3, address, buf, 1, HAL_MAX_DELAY);
	if( retStatus != HAL_OK ){
		return -1;
	}

	address = (I2C_SLAVE_ID | I2C_READ);
	retStatus = HAL_I2C_Master_Receive(&hi2c3, address, buf, 1, HAL_MAX_DELAY);
	if( retStatus != HAL_OK ){
		return -1;
	}
...
HAL_StatusTypeDef pulseOximeter_writeRegister(uint8_t reg, uint8_t value)
{
	...
	uint8_t address = (I2C_SLAVE_ID | I2C_WRITE);
	retStatus = HAL_I2C_Master_Transmit(&hi2c3, address, buf, 2, HAL_MAX_DELAY);
...
FIFO_LED_DATA pulseOximeter_readFifo(void)
{
	...
	HAL_I2C_Master_Transmit(&hi2c3, address, buf, 1, HAL_MAX_DELAY);

	address = (I2C_SLAVE_ID | I2C_READ);
	HAL_I2C_Master_Receive(&hi2c3, address, buf, numBytes, HAL_MAX_DELAY);
...
int8_t pulseOximeter_readFIFO(uint8_t* dataBuf, uint8_t numBytes)
{
	...
	address = (I2C_SLAVE_ID | I2C_READ);
	retStatus = HAL_I2C_Master_Receive(&hi2c3, address, buf, numBytes, HAL_MAX_DELAY);
	if( retStatus != HAL_OK ){
		return -1;
	}
...
{
    printf("Temperature: %.2f", (pulseOximeter.temperature));
    printf("  Heart Rate(BPM): %.2f", (pulseOximeter.heartBPM));
    printf("  Oxygen Saturation SpO2(%): %.3f\n\r", (pulseOximeter.SpO2));
